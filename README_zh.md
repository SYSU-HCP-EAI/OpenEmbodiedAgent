...
*   🧪 **Runtime Session Loop**: 以 session 为中心的 runtime（`TARGETS.md` / `SKILLS.md` / `SESSIONS.md` → `WatchdogSupervisor` → preflight → `SessionRunner` → `TargetSessionHandle` → skill runtime → artifacts）。当前 supervisor 为串行模式，会按 priority 选择 pending session，在进入 `running` 前执行配置与 contract 的 strict preflight，并将结果与可复用 preflight 失败经验写回 `SESSIONS.md` 和 `LESSONS.md`。
