...
当前新框架支持 DummySimTarget、DummyOpenPIAdapter、DummyPolicyClient 调试，并已具备组件化串行 runtime watchdog：支持 session 调度、preflight 健康检查、runtime registry、retry/failure escalation。
...
*   🧪 **Runtime Session Loop**: 新增以 session 为中心的 runtime（`SESSIONS.md` → 组件化 `WatchdogSupervisor` → policy client → simulation target → artifacts）。当前 supervisor 为串行模式，会按 priority 选择 pending session，执行 preflight 检查，并将 retry/failure 状态写回 `SESSIONS.md`。
...
Runtime 使用 `TARGETS.md`、`SKILLS.md`、`SESSIONS.md`，不走旧的 `ACTION.md` 队列。组件化 watchdog 没有改变启动命令，仍然使用 `scripts/run_runtime_watchdog.py`。
...
```
成功后，被选中的 pending session 会标记为 `succeeded`，episode 摘要会写入 `artifacts/runtime/<session_id>/episode.json`。如果存在多个 pending session，串行 scheduler 会按 `high > normal > low` 选择，同优先级保持文件顺序。
