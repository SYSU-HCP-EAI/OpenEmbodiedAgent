...
当前新框架支持以 session 为中心的 runtime，并提供 DummySimTarget、DummyOpenPIAdapter、DummyPolicyClient 轻量 smoke 链路。串行 `WatchdogSupervisor` 会通过工作区锁 claim session，执行 strict compatibility preflight，解析 `AdapterPlan`，监督执行，并把结果、经验和 artifacts 写回工作区。Runtime target、skill runtime、adapter 与 bridge 通过轻量 Python registry 扩展；adapter 与 bridge 标识使用 `target_adapter://`、`policy_adapter://`、`bridge://` 等显式 URI 命名空间。
...
*   🧪 **Runtime Session Loop**: 以 session 为中心的 runtime（`TARGETS.md` / `SKILLS.md` / `SESSIONS.md` → `WatchdogSupervisor` → `AdapterPlan` → target/policy execution → artifacts）。当前 supervisor 为串行模式，会按 priority 选择 pending session，在进入 `running` 前执行配置与 contract 的 strict preflight，并将结果与可复用 preflight 失败经验写回 `SESSIONS.md` 和 `LESSONS.md`。
...
Runtime 使用 `TARGETS.md`、`SKILLS.md`、`SESSIONS.md` 以及 `configs/runtime/` 下的外部 YAML，不走旧的 `ACTION.md` 队列。`TARGETS.md` 指向 target runtime endpoint、target adapter、sensor config 与 runtime contract；`SKILLS.md` 声明 runtime 与 policy adapter 需求；`SESSIONS.md` 只选择 target 和 skill，实际 adapter plan 由 preflight 解析。Sensor YAML 与 perception YAML 会在执行前校验；真实 target observation 的 channel、dtype、shape 会在 runtime 读取 observation 进行环境刷新或技能执行时校验。
